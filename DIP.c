	...
	for (int i = 2; i < hInfo.biHeight - 2; i++)
		for (int j = 2; j < hInfo.biWidth - 2; j++) {
			...
			for (int k = -2; k <= 2; k++) {
				for (int l = -2; l <= 2; l++)
				{
					avg += lpImg[(i + k)*rwsize + (j + l)];
					max = ismax((int)lpImg[(i + k)*rwsize + (j + l)], max);
				}
			}
			avg = avg / 25;
			outImg1[i*rwsize + j] = ((max - lpImg[i*rwsize + j]) / avg);
			outImg1[i*rwsize + j] = outImg1[i*rwsize + j] * avg*avg / max / lpImg[i*rwsize + j];
		//	printf("%f  ", outImg1[i*rwsize + j]);
		}
	max = 0;
	for (int i = 2; i < hInfo.biHeight - 2; i++)
		for (int j = 2; j < hInfo.biWidth - 2; j++) {
			max = ismax((int)outImg1[(i )*rwsize + (j)], max);
		
		}	
	
	for (int i = 2; i < hInfo.biHeight - 2; i++)
		for (int j = 2; j < hInfo.biWidth - 2; j++) {
			outImg[i*rwsize + j] = (unsigned char)(outImg1[i*rwsize + j] / max * 255);
